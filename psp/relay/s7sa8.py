...
def zone_7sa8(
    ang: float, X: float, R: float, inclination: float, DirMode: str = "Forward"
):
    """
    ...
    if DirMode not in ["Forward", "Reverse", "Non-directional"]:
        raise ValueError(
            '"DirMode" must be either "Forward", "Reverse" or "Non-directional"'
        )
